...
from text import (
    ...
    warn_message,
    kick_message,
    no_event_url_message,
    unsupported_event_url_message,
    event_created_message,
    event_creation_error_message,
    admin_access_error_message,
    queue_user_not_found_message,
    guest_list_success_message,
    kick_message,
    upcoming_events_header,
    no_upcoming_events_message,
    duplicate_event_message,
    duplicate_event_question_message,
    duplicate_event_create_button_text,
...
async def new_member_welcome_command(
...
async def whois_reply_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success:
        mention = get_mention(
            member_id,
            get_name(update.effective_user.first_name, update.effective_user.last_name),
        )
...
async def unset_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        if job_removed
        else "You have no active update timer."
    )
    await update.effective_message.reply_text(text)
...
async def create_event_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    is_valid, sanitized_url, error_msg, domain = validate_and_sanitize_url(
        url, allowed_domains
    )

    if not is_valid:
        logger.warning(
            f"Invalid URL provided by user {username} (ID: {user_id}), domain: {domain or 'unknown'}. Error: {error_msg}"
        )
    ...
    logger.info(
        f"Valid URL from user {username} (ID: {user_id}): {sanitized_url}, domain: {domain}"
    )
                ...
                [
                    [
                        InlineKeyboardButton(
                            duplicate_event_create_button_text,
                            callback_data="duplicate_event_create",
                        ),
                        InlineKeyboardButton(
                            duplicate_event_skip_button_text,
                            callback_data="duplicate_event_skip",
                        ),
                    ]
                ]
            ),
...
async def button_click_handler(
    ...
    if query.data == "duplicate_event_skip":
        message = query.message.text.replace(
            duplicate_event_question_message, duplicate_event_message
        )
...
async def guest_list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success:
        await update.effective_message.reply_html(
            guest_list_success_message.format(name=mention)
        )
        await clean_up_welcome_message(context, update.effective_chat.id, user_id)
        await clean_up_warn_message(context, update.effective_chat.id, user_id)
...
async def kick_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success:
        await context.bot.send_message(
            chat_id=update.effective_chat.id, text=kick_message.format(name=mention)
        )
        await clean_up_welcome_message(context, update.effective_chat.id, user_id)
        await clean_up_warn_message(context, update.effective_chat.id, user_id)
        ...
        await context.bot.ban_chat_member(
            chat_id=update.effective_chat.id, user_id=user_id, until_date=until_date
        )
...
async def kick_idle(context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.unban_chat_member(
        chat_id=context.job.chat_id, user_id=context.job.user_id, only_if_banned=True
    )
...
async def clean_up_warn_message(
    context: ContextTypes.DEFAULT_TYPE, chat_id: int, user_id: int
):
...
def is_old_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    difference = abs(
        update.message.date - datetime.datetime.now(update.message.date.tzinfo)
    )
...
if __name__ == "__main__":
    ...
    application.add_handler(
        MessageHandler(
            filters.StatusUpdate.NEW_CHAT_MEMBERS, new_member_welcome_command
        )
    )

    application.add_handler(
        MessageHandler(filters.StatusUpdate.LEFT_CHAT_MEMBER, member_left_command)
    )

    whois_handler = MessageHandler(
        filters.Regex(r"(?i)(^|\s+)#whois($|\s+)"), whois_reply_command
    )
